...
class TradeController extends Controller
{
    ...
    public function actionRetriveReceivecardRecord(){
        ...
        $resp = new  M2B_RetriveReceiveCardRecord_Response();
        $errorCode = 0;
        $mallItems = [];
        if(HttpUtils::parseNetMsg($resp,$respMsgStream,$errorCode)){
...
//		$req->setMoney($_GET['money']);
		...
		$this->reqHandler->setParameter('notify_url','http://www.xiaoyougames.com/GameManager/frontend/web/index.php?r=trade/pay-call-back');//
	...
	public function actionPayCallBack(){
        ...
        if($this->resHandler->isTenpaySign()){
            if($this->resHandler->getParameter('status') == 0 && $this->resHandler->getParameter('result_code') == 0){
				...
				$req->setMoney($this->resHandler->getParameter('total_fee'));   
				$req->setChannel(1);  
